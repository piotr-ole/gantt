...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "gantt", "tests")); system.time(test_check("gantt", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran gantt_$(VERSION).tar.gz
	rm -rf gantt.Rcheck/
